...
def getURL(topic, scroll_down): #scroll_down
    ...
    df.to_csv(topic+'~~Sd'+scroll_down+'.csv',index=False)
    # print('done')
    
    url = 'https://www.youtube.com/results?search_query=%23'+topic+update_time
    r = s.get(url)
    # print(r)
    print('start render..')
    r.html.render(sleep = 3, timeout = 100, keep_page = True, scrolldown = int(scroll_down))
    ...
    for elem in container :
        ...
        # print(vid_title)
        vid_url = 'https://www.youtube.com' + elem.find('a#video-title', first = True).attrs['href']
        data23.append([vid_title, vid_url])
    # print('done for loop')
    
    df = pd.DataFrame(data23, columns=['V_title', 'V_url'])
    df.to_csv(topic+'#~~Sd'+scroll_down+'.csv',index=False)
    # print('done')
